...
def quality_view_model(analysis: dict[str, Any] | None, response: dict[str, Any] | None) -> QualityViewModel:
...
def summary_metrics(summary: dict[str, Any]) -> list[SummaryMetric]:
    ...
    avg_rom_str = f"{avg_rom:.3f}" if isinstance(avg_rom, (int, float)) else "n/a"

    return [
        SummaryMetric(label="Reps", value=summary.get("n_reps", "n/a")),
        SummaryMetric(label="Avg ROM", value=avg_rom_str),
        SummaryMetric(label="Low confidence reps", value=summary.get("n_low_confidence", "n/a")),
    ]
...
def top_fault_rows(summary: dict[str, Any], limit: int = 5) -> list[str]:
    ...
    for tf in faults[:limit]:
        if isinstance(tf, dict):
            rows.append(f"- {tf.get('code', 'UNKNOWN')} × {tf.get('count', 0)} (max severity: {tf.get('severity_max', 'info')})")
...
def artifact_analysis_json_path(analysis: dict[str, Any] | None) -> Path | None:
    ...
    if analysis_json_path and Path(str(analysis_json_path)).exists():
        return Path(str(analysis_json_path))
    return None


def resolve_overlay_path(payload: dict[str, Any] | None, analysis: dict[str, Any] | None) -> Path | None:
    raw = [
