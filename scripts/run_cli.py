...
def main():
    ...
    per_rep = result.get("per_rep_feedback") or result.get("rep_feedback")
    if isinstance(per_rep, list) and per_rep:
        print()
        print("Per-rep feedback:")
        for i, fb in enumerate(per_rep, start=1):
            text = fb
            # Allow for dict-based feedback structures, just in case
            if isinstance(fb, dict):
                text = fb.get("text") or fb.get("summary") or str(fb)
            print(f"  Rep {i}: {text}")
